...
EndPoint.prototype.sendNextVersion = function(waitWSResponse) {
    ...
    if (DOUT) debug('Sending req #%d on %s, qs=%s', this.requestCount, this.channelID, data);
        ...
        res.once('end', function() {
            ...
            if(res.statusCode !== 200) {
                ...
                if (self.version === self.expectedVersion) {
                    if (DOUT) debugTimeout("Skip timeout creation, version already updated");
                    ...
                    self.timeoutID = setTimeout(function() {
                        if (DOUT) debugTimeout("Timed out. ver: %d, expected: %d", 
                            self.version,

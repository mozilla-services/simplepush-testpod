...
function Server(http, serverList, minUpdateTime, maxUpdateTime, waitTimeout) {
...
Server.prototype.sendUpdate = function(endpoint) {
    ...
    var serverName = this.serverList[Math.floor(random(0, this.numServers))];
    var opts = {
        hostname: serverName,
        port: u.port,
        method: 'PUT',
    ...
    if (DOUT) {
        if (u.hostname == serverName) {
            debugSend('PUT *SAME* server,  %s = %d', endpoint.channelID, endpoint.expectedVersion);
        } else {
            debugSend('PUT *DIFF* server,  %s = %d', endpoint.channelID, endpoint.expectedVersion);
        }
    }

    var req = this.http.request(opts, function(res) {
        if (DOUT) debugSend("RES %s, %s", endpoint.channelID, res.statusCode);
